...
IWDDIR="/var/lib/iwd/"
declare -A networks
networks=(["-e"]=eduroam.8021x ["-s"]=Southeastern_WiFi.open ["-p"]=CompEng0001.psk ["-m"]=makerspace.psk ["-M"]=MACKNADE-FREE.open ["-n"]="NHS Wi-Fi.open" ["-f"]=HometelecomD2MZ5G.psk ["-r"]=BT-FNASJ9.psk ["-S"]="Pretty fly for a Wi Fi.psk")
...
edit(){
	sudo vim "${IWDDIR}${networks[$1]}"
}

print(){
	sudo bat "${IWDDIR}${networks[$1]}" | grep -w Passphrase |  awk -F "=" '{print$2}'
}
